...
[latCoords, latVals] = readLAT(os.path.join(dataDir, latFile))
...
# For colorbar ranges
MINLAT = math.floor(min(frontLAT)/10)*10
MAXLAT = math.ceil(max(frontLAT)/10)*10
...
pos = ax.scatter(frontCoord[:,0], frontCoord[:,1], frontCoord[:,2], c=frontLAT, cmap='rainbow_r', vmin=MINLAT, vmax=MAXLAT, s = 5)	
