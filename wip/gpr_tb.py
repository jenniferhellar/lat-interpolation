...
if NUM_TEST_SAMPLES > M:
	print('Not enough known samples for this experiment. Please reduce NUM_TEST_SAMPLES.\n')
	exit(1)
...
for test in range(NUM_TEST_REPEATS):
	...
	# calculate error on the test samples
	...
	# plt.colorbar(pos, cax=cax, label='mapLAT (ms)', orientation="horizontal")
	...
	# filename = os.path.join('results_gpr', 'p{}_t{:g}_m{:g}_test{:g}_entire.png'.format(patient, EDGE_THRESHOLD, NUM_TEST_SAMPLES, test))
	...
	truthFilename = os.path.join('results_gpr', 'gpr_p{}_t{:g}_m{:g}_truth.png'.format(patient, EDGE_THRESHOLD, NUM_TEST_SAMPLES, test))
	...
	filename = os.path.join('results_gpr', 'gpr_p{}_t{:g}_m{:g}_test{:g}_known.png'.format(patient, EDGE_THRESHOLD, NUM_TEST_SAMPLES, test))
	fig.savefig(filename)
...
filename = os.path.join('results_gpr', 'gpr_p{}_t{:g}_m{:g}_tests{:g}_metrics.txt'.format(patient, EDGE_THRESHOLD, NUM_TEST_SAMPLES, NUM_TEST_REPEATS))
fid = open(filename, 'w')
fid.write('{:<30}{}\n'.format('file', nm))
fid.write('{:<30}{:g}\n'.format('n', n))
fid.write('{:<30}{:g}\n\n'.format('m', M))
