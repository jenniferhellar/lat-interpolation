...
if not os.path.isdir('results_comparison'):
...
""" MAGIC-LAT estimate """
latEst = magicLATwavelet(coordinateMatrix, connectivityMatrix, EDGES, TrIdx, TrCoord, TrVal, EDGE_THRESHOLD)
...
# # Save the figure
# filename = os.path.join('results_comparison', 'p{}_t{:g}_m{:g}_testpts.png'.format(patient, EDGE_THRESHOLD, NUM_TRAIN_SAMPS))
...
thisAx.set_title('Ground truth (back)\n')
thisAx.set_xlabel('X', fontweight ='bold') 
thisAx.set_ylabel('Y', fontweight ='bold') 
thisAx.set_zlabel('Z', fontweight ='bold')
thisAx.view_init(elev, azim)
...
# Plot 5: MAGIC-LAT output signal (rotated)
thisAx = axes[4]
...
# Plot 6: GPR output signal (rotated)
thisAx = axes[5]
...
# # Save the figure
# filename = os.path.join('results_comparison', 'p{}_t{:g}_m{:g}_entire.png'.format(patient, EDGE_THRESHOLD, NUM_TRAIN_SAMPS))
# fig.savefig(filename)
# plt.close()
