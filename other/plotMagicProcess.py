...
MAXLAT = MINLAT + math.ceil((7/8 * (max(allLatVal) - MINLAT)) / 10)*10
...
origLatPoints = Points(OrigLatCoords, r=10).cmap('gist_rainbow', OrigLatVals, vmin=MINLAT, vmax=MAXLAT).addScalarBar(c='white', title='LAT (ms)   ', titleFontSize=fontSize, size=size)
allLatPoints = Points(allLatCoord, r=10).cmap('gist_rainbow', allLatVal, vmin=MINLAT, vmax=MAXLAT).addScalarBar(c='white', title='LAT (ms)   ', titleFontSize=fontSize, size=size)
latPoints = Points(latCoords, r=10).cmap('gist_rainbow', latVals, vmin=MINLAT, vmax=MAXLAT).addScalarBar(c='white', title='LAT (ms)   ', titleFontSize=fontSize, size=size)
trPoints = Points(TrCoord, r=10).cmap('gist_rainbow', TrVal, vmin=MINLAT, vmax=MAXLAT).addScalarBar(c='white', title='LAT (ms)   ', titleFontSize=fontSize, size=size)
nnLatPoints = Points(vertices, r=10).cmap('gist_rainbow', latNN, vmin=MINLAT, vmax=MAXLAT).addScalarBar(c='white', title='LAT (ms)   ', titleFontSize=fontSize, size=size)
latEstPoints = Points(vertices, r=10).cmap('gist_rainbow', latEst, vmin=MINLAT, vmax=MAXLAT).addScalarBar(c='white', title='LAT (ms)   ', titleFontSize=fontSize, size=size)
...
vplt.screenshot(filename=os.path.join(outSubDir, '0_all'), returnNumpy=False).close()

vplt = Plotter(N=1, axes=0, offscreen=True)
vplt.show(mesh, origLatPoints, azimuth=a, elevation=e, roll=r, bg='black', zoom=z)
vplt.screenshot(filename=os.path.join(outSubDir, '0_orig'), returnNumpy=False).close()
...
coloredMesh.interpolateDataFrom(latEstPoints, N=1).cmap('gist_rainbow', vmin=MINLAT, vmax=MAXLAT).addScalarBar(c='white', title='LAT (ms)   ', titleFontSize=fontSize, size=size)
