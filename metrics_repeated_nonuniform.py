...
for test in range(NUM_TEST_REPEATS):
	...
	trueColors = cmap(norm(np.array([TstVal])))[:, :, :3]
	trueColors = cv2.cvtColor(trueColors.astype("float32") / 255, cv2.COLOR_RGB2LAB)

	magicColors = cmap(norm(np.array(latEst[TstIdx])))[:, :, :3]
	magicColors = magicColors.reshape((1, max(magicColors.shape), 3))
	magicColors = cv2.cvtColor(magicColors.astype("float32") / 255, cv2.COLOR_RGB2LAB)

	dE1976 = delta_e(trueColors, magicColors)
	dE2000 = np.mean(colour.delta_E(trueColors, magicColors, method='CIE 2000'))

	gprColors = cmap(norm(np.array([latEstGPR[TstIdx]])))[:, :, :3]
	gprColors = cv2.cvtColor(gprColors.astype("float32") / 255, cv2.COLOR_RGB2LAB)

	dE1976GPR = delta_e(trueColors, gprColors)
	dE2000GPR = np.mean(colour.delta_E(trueColors, gprColors, method='CIE 2000'))

	quLATiColors = cmap(norm(np.array([latEstquLATi[TstIdx]])))[:, :, :3]
	quLATiColors = cv2.cvtColor(quLATiColors.astype("float32") / 255, cv2.COLOR_RGB2LAB)

	dE1976quLATi = delta_e(trueColors, quLATiColors)
	dE2000quLATi = np.mean(colour.delta_E(trueColors, quLATiColors, method='CIE 2000'))
